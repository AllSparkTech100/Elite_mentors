function Navbar() {
    ...
    const getNavLinkClass = ({ isActive }) =>
        `transition-all duration-200 text-sm md:text-[15px] font-medium ${isActive
            ? "text-blue-600"
            : "text-slate-600 hover:text-blue-600"
        }`;

    const getMobileNavLinkClass = ({ isActive }) =>
        `block text-base font-medium transition-colors ${isActive ? "text-blue-600" : "text-slate-700 hover:text-blue-600"
        }`;
    return (
        <div>
            {/* Floating Navbar */}
